...
func maketree1() {
	...
	tree1bound = byte(getbits(5))
	mindepth = byte(getbits(3))
	if mindepth == 0 {
...
func maketree2(tree2bound int) { /* in use: 5 <= tree2bound <= 8 */
...
func tree1Get() int {
	return treeGet(&tree1)
}

func tree2Get() int {
	return treeGet(&tree2)
}
